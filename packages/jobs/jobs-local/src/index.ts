 ...
 * @module @buddhilive/dsh-jobs-local
 */

import { Context } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import type { Agent } from '@buddhilive/dsh-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@buddhilive/dsh-scope'
import type { ScopeLayer } from '@buddhilive/dsh-scope'
import { deadline, timeoutOf } from '@buddhilive/dsh-timeout'
import { JobRegistry, JobId } from '@buddhilive/dsh-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@buddhilive/dsh-jobs'
 ...
 * `@buddhilive/dsh-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @buddhilive/dsh-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @buddhilive/dsh-agent)')
    }
