...
describe('generated tsconfig package aliases', () => {
  it('maps each package to its own source directory', () => {
    ...
    const session = aliases.find(alias => alias.specifier === '@buddhilive/dsh-session')
    expect(session).toEqual({
      specifier: '@buddhilive/dsh-session',
    ...
    expect(aliases.some(alias => alias.specifier === '@buddhilive/dsh-typert-protocol')).toBe(false)
  })

  it('yields to a hand-written alias and closes without a trailing comma', () => {
    const aliases = [
      { specifier: '@buddhilive/dsh-a', source: './packages/g/a/src', hasInvariant: true },
      { specifier: '@buddhilive/dsh-b', source: './packages/g/b/src', hasInvariant: false },
    ]
    const body = renderAliases(aliases, new Set(['@buddhilive/dsh-a']))

    // The hand-written bare alias is skipped; its /invariant sibling is not.
    expect(body).toBe([
      '      "@buddhilive/dsh-a/invariant": ["./packages/g/a/src/invariant.ts"]',
      '      "@buddhilive/dsh-b": ["./packages/g/b/src"]',
    ].join(',\n'))
  ...
  it('names a package that no alias covers', () => {
    ...
    expect(uncoveredPackages(
      ['@buddhilive/dsh-a', '@buddhilive/dsh-b'],
      new Set(['@buddhilive/dsh-a', '@buddhilive/dsh-a/invariant']),
    )).toEqual(['@buddhilive/dsh-b'])

    expect(uncoveredPackages(['@buddhilive/dsh-a'], new Set(['@buddhilive/dsh-a']))).toEqual([])
  })
  ...
  it('covers every workspace package in the committed config', () => {
    ...
    expect(names).toContain('@buddhilive/dsh-typert-protocol')
  ...
  it('leaves no wildcard that probes every package group', () => {
    ...
    expect(config).not.toContain('"@buddhilive/dsh-*":')
    expect(config).not.toContain('"@buddhilive/dsh-*/invariant":')
  })
})
