...
describe('BootPage', () => {
  ...
  it('lists failed entries', () => {
    const { el, page } = mount()
    page.setState('@buddhilive/dsh-client-ui-layout', 'failed')
    page.setState('ok', 'active')
    page.setState('@buddhilive/dsh-client-ui-tool', 'failed')
    expect(el.textContent).toContain('@buddhilive/dsh-client-ui-layout')
    expect(el.textContent).toContain('@buddhilive/dsh-client-ui-tool')
