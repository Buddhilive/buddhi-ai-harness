...
function declaration(
  ...
  return {
    name: short.startsWith('@') ? short : '@buddhilive/dsh-client-' + short,
...
describe('source package uses', () => {
  it('counts type imports, module augmentations, dynamic imports, and JSX', () => {
    const uses = collectSourcePackageUses('feature.tsx', [
      "import type { A } from '@buddhilive/dsh-a/subpath'",
      "declare module '@buddhilive/dsh-client-ui-slots' {}",
      "const load = () => import('@buddhilive/dsh-b/remote')",
    ...
    expect([...uses].sort()).toEqual([
      '@buddhilive/dsh-a',
      '@buddhilive/dsh-b',
      '@buddhilive/dsh-client-ui-slots',
      'react',
    ])
    expect([...collectRuntimeSourcePackageUses('feature.tsx', [
      "import type { A } from '@buddhilive/dsh-a/subpath'",
      "declare module '@buddhilive/dsh-client-ui-slots' {}",
      "const load = () => import('@buddhilive/dsh-b')",
      'export const view = <div />',
    ].join('\n'))].sort()).toEqual([
      '@buddhilive/dsh-b',
      'react',
...
describe('package modes', () => {
  ...
  it('requires every preloaded external to have a parser preload row', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      + '"@buddhilive/dsh-client-bootstrap/client" has no matching PARSER_PRELOAD_IDS row in '
...
describe('module requests', () => {
  it('rejects runtime requests from one client feature package to another dynamic row', () => {
    const ui = declaration('ui', {
      external: ['@buddhilive/dsh-client-slots/client'],
      runtimeSourceUses: {
        '@buddhilive/dsh-client-slots': ['packages/client/ui/src/client/index.ts'],
      },
    })
    ...
    expect(collectClientPackageViolations(facts([], { declarations: [ui, slots] }))).toEqual([
      ...
      + '"@buddhilive/dsh-client-slots/client"; import shared types only or call an injected Cordis service',
    ])
  })

  it('rejects stale externals and accepts a runtime import outside client feature packages', () => {
    const gateway = {
      ...declaration('@buddhilive/dsh-api-gateway'), manifest: 'packages/api/gateway/package.json',
    }
    const stale = { ...declaration('@buddhilive/dsh-api-stale', {
      external: ['@buddhilive/dsh-api-gateway/client'],
    }), manifest: 'packages/api/stale/package.json' }
    const live = { ...declaration('@buddhilive/dsh-api-live', {
      external: ['@buddhilive/dsh-api-gateway/client'],
      runtimeSourceUses: {
        '@buddhilive/dsh-api-gateway': ['packages/api/live/src/client/index.ts'],
      },
      runtimeSourceSpecifiers: {
        '@buddhilive/dsh-api-gateway/client': ['packages/api/live/src/client/index.ts'],
      },
    }), manifest: 'packages/api/live/package.json' }
    expect(collectClientPackageViolations(facts([], {
      declarations: [gateway, stale, live],
    }))).toEqual([
      stale.manifest + ': dsh.client.external "@buddhilive/dsh-api-gateway/client"'
  ...
  it('requires the exact external subpath to be imported at runtime', () => {
    ...
    const subject = { ...declaration('@buddhilive/dsh-api-session-controller', {
      external: ['@buddhilive/dsh-api-gateway/client'],
      runtimeSourceUses: {
        '@buddhilive/dsh-api-gateway': ['packages/api/session-controller/src/client/index.ts'],
      },
      runtimeSourceSpecifiers: {
        '@buddhilive/dsh-api-gateway/remote': ['packages/api/session-controller/src/client/index.ts'],
      },
    ...
    expect(collectClientPackageViolations(facts([], { declarations: [gateway, subject] }))).toEqual([
      subject.manifest + ': dsh.client.external "@buddhilive/dsh-api-gateway/client"'
      + ' has no runtime import or re-export in production source; remove the stale declaration',
    ])
  })
  ...
  it('rejects duplicates, empty values, self-requests, and missing suppliers', () => {
    const ui = declaration('ui', {
      external: ['', '@buddhilive/dsh-client-ui', '@buddhilive/dsh-missing', '@buddhilive/dsh-missing'],
      inject: ['', '@buddhilive/dsh-a', '@buddhilive/dsh-a'],
    })
  ...
  it('rejects synchronous module-request cycles but ignores inject cycles', () => {
    const a = { ...declaration('@buddhilive/dsh-api-a', {
      external: ['@buddhilive/dsh-api-b'],
      inject: ['@buddhilive/dsh-api-b'],
      runtimeSourceUses: { '@buddhilive/dsh-api-b': ['packages/api/a/src/client.ts'] },
      runtimeSourceSpecifiers: { '@buddhilive/dsh-api-b': ['packages/api/a/src/client.ts'] },
    }), manifest: 'packages/api/a/package.json' }
    const b = { ...declaration('@buddhilive/dsh-api-b', {
      external: ['@buddhilive/dsh-api-a'],
      inject: ['@buddhilive/dsh-api-a'],
      runtimeSourceUses: { '@buddhilive/dsh-api-a': ['packages/client/b/src/client.ts'] },
      runtimeSourceSpecifiers: { '@buddhilive/dsh-api-a': ['packages/client/b/src/client.ts'] },
...
describe('manifest declarations', () => {
  ...
  it('fixes malformed declaration entries without changing dependency sections', () => {
    ...
    const subject = pkg('feature', {
      external: ['', 'react', '@buddhilive/dsh-client-feature', '@buddhilive/dsh-missing'],
      inject: ['', '@buddhilive/dsh-agent', '@buddhilive/dsh-agent'],
      sourceUses: {
        '@buddhilive/dsh-agent': ['packages/client/feature/src/index.ts'],
        '@buddhilive/dsh-client-ui-slots': ['packages/client/feature/src/view.tsx'],
      },
      dependencies: {
        [CORDIS]: 'workspace:^',
        '@buddhilive/dsh-agent': 'workspace:*',
      },
      peerDependencies: {
        '@buddhilive/dsh-client-ui-slots': 'workspace:^',
    ...
    expect(fixed.dsh.client).toMatchObject({
      external: ['@buddhilive/dsh-missing'],
      inject: ['@buddhilive/dsh-agent'],
    })
