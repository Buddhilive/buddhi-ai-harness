...
import { SessionHistoryController } from '@buddhilive/dsh-api-session-controller/src/history.ts'
import { subagentIdentityProjectionDefinition } from '@buddhilive/dsh-subagent/src/projection.ts'
import TypertRegistry from '@buddhilive/dsh-typert-registry'
import { createUserMessage, MessageId } from '@buddhilive/dsh-llm'
...
import {
  ...
  type StoredPrefix,
} from '@buddhilive/dsh-session-persistence'
...
describe('Remote Agent and Session lookup policy', () => {
  it('deduplicates a cold resume across Agent and Session parameters', async () => {
    ...
    const resumedSession = { id: sessionId, header: meta, events: [] } as unknown as import('@buddhilive/dsh-session').Session
