...
## Do not import `@buddhilive/dsh-client-*` here
...
`@buddhilive/dsh-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@buddhilive/dsh-client-web` and the boot-manifest
type from `@buddhilive/dsh-client-modules/client`: booting the real shell is what
