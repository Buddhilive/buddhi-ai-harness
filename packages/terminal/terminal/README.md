...
Mount `@buddhilive/dsh-terminal` whenever a composition needs terminal sessions whose state survives across tool calls. The service alone does nothing useful: pair it with a backend such as `@buddhilive/dsh-terminal-bash` and a tool package such as `@buddhilive/dsh-tool-terminal`, and load all three in one composition.
...
Nothing directly. This package registers no prompt or tool; `@buddhilive/dsh-tool-terminal` owns visible schemas and result text.
...
No direct invalidation; `@buddhilive/dsh-tool-terminal` owns request-prefix changes.
