 ...
 *
 * @module @buddhilive/dsh-api-settings-controller/src/credentials.ts
 */

import { Context } from '@deepseek-ai/cordis'
import { credentialRef } from '@buddhilive/dsh-credentials'
import type { CredentialProvider } from '@buddhilive/dsh-credentials'
import type { CredentialInfo } from '@buddhilive/dsh-credentials/types'
import { Remote, RemoteError, TypertRemoteService } from '@buddhilive/dsh-typert-protocol'
import { z } from 'zod'
  ...
  private provider(): CredentialProvider {
    ...
    if (credentials === undefined) {
      throw new RemoteError(
        'gateway/internal',
        'credentials service is absent: this deployment does not mount a credential provider (e.g. @buddhilive/dsh-credentials-local) in its composition',
        {},
      )
    }
    return credentials
