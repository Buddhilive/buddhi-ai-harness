...
import FileSettingsProvider from '@buddhilive/dsh-settings-file'
import { getOrCreateAnonymousUserId } from '@buddhilive/dsh-anonymous-user-id'
...
import * as DeepSeekPluginPackageInventory from '@buddhilive/dsh-plugin-package-inventory-deepseek'
...
async function loadComposition(
  ...
  await writeFile(configPath, [
    '- id: llm',
    "  name: '@buddhilive/dsh-llm'",
    '- id: session',
    "  name: '@buddhilive/dsh-session'",
    '- id: agents',
    "  name: '@buddhilive/dsh-agent'",
    '- id: deepseek-llm-api-extensions',
    "  name: '@buddhilive/dsh-deepseek-llm-api-extensions'",
    '- id: session-log-deepseek',
    "  name: '@buddhilive/dsh-session-log-deepseek'",
    ...options.enableSessionLog === true
      ? ['  config:', '    enabled: true']
      : [],
    '- id: plugin-package-inventory-deepseek',
    "  name: '@buddhilive/dsh-plugin-package-inventory-deepseek'",
    ...options.withDynamic
      ? [
        '- id: settings',
        "  name: '@buddhilive/dsh-settings-file'",
        '  config:',
      ...
      : [],
    '- id: llm-deepseek',
    "  name: '@buddhilive/dsh-llm-deepseek'",
    '  config:',
  ...
  const modules = new Map<string, unknown>([
    ['@buddhilive/dsh-llm', LlmRuntime],
    ['@buddhilive/dsh-session', SessionStore],
    ['@buddhilive/dsh-agent', AgentRegistry],
    ['@buddhilive/dsh-deepseek-llm-api-extensions', DeepSeekLlmApiExtensionRegistry],
    ['@buddhilive/dsh-session-log-deepseek', SessionLogDeepSeek],
    ['@buddhilive/dsh-plugin-package-inventory-deepseek', DeepSeekPluginPackageInventory],
    ['@buddhilive/dsh-settings-file', FileSettingsProvider],
    ['@buddhilive/dsh-credentials-local', LocalCredentialProvider],
    ['@buddhilive/dsh-llm-deepseek', LlmDeepSeek],
  ])
...
describe('llm-deepseek real dynamic composition', () => {
  it('keeps session upload off and package inventory on by default in the real Loader composition', async () => {
    ...
    expect(request.dsh_plugin_packages.packages).toEqual(expect.arrayContaining([
      { name: '@buddhilive/dsh-deepseek-llm-api-extensions', version: '0.1.0-rc.8' },
      { name: '@buddhilive/dsh-llm-deepseek', version: '0.1.0-rc.8' },
      { name: '@buddhilive/dsh-session-log-deepseek', version: '0.1.0-rc.8' },
    ]))
