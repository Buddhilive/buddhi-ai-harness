...
```yaml
- name: '@buddhilive/dsh-permission-presets'
  config:
    presets:
      workspace-write:
...
The generated [configuration catalog](../../../docs/config-catalog.md#buddhilivedsh-permission-presets) is the exhaustive source for every accepted field and its JSDoc. The name `custom` is reserved for the derived not-a-preset state and cannot name a table entry. Mounting requires a confining bash executor (one that reports a `sandboxMode`) and the approval service.
