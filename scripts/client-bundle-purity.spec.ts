...
const REQUESTING_PACKAGE = '@buddhilive/dsh-client-ui-conversation'
...
describe('client bundle build faces', () => {
  it('watches source in development and consumes emitted JavaScript in the Client build', () => {
    const bundle = clientBundle('@buddhilive/dsh-client-test', ['lib/types/index.js'])
...
describe('client bundle purity gate', () => {
  ...
  it('leaves default externals and non-scoped specifiers alone', () => {
    expect(resolveId('@buddhilive/dsh-client-store')).toBeNull()
    expect(resolveId('@buddhilive/dsh-client-ui-slots')).toBeNull()
    expect(resolveId('@buddhilive/dsh-client-ui-primitives')).toBeNull()
    expect(resolveId('react')).toBeNull()
    expect(resolveId('zod')).toBeNull()
  })

  it('rejects the retired web-react platform package', () => {
    expect(() => resolveId('@buddhilive/dsh-client-web-react')).toThrow(/purity/)
    expect(() => resolveId('@buddhilive/dsh-client-web-react/store')).toThrow(/purity/)
  })

  it('lets inline-safe libraries inline', () => {
    expect(resolveId('@buddhilive/dsh-session/surface')).toBeNull()
    expect(resolveId('@buddhilive/dsh-brand')).toBeNull()
    expect(resolveId('@buddhilive/dsh-deque')).toBeNull()
    expect(resolveId('@buddhilive/dsh-util-values')).toBeNull()
    expect(resolveId('@buddhilive/dsh-token-meter/client')).toBeNull()
    expect(() => resolveId('@buddhilive/dsh-token-meter')).toThrow(/purity/)
    expect(() => resolveId('@buddhilive/dsh-token-meter/client/internal')).toThrow(/purity/)
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@buddhilive/dsh-goal/remote')).toBeNull()
    expect(() => resolveId('@buddhilive/dsh-goal')).toThrow(/purity/)
    expect(() => resolveId('@buddhilive/dsh-goal/client')).toThrow(/purity/)
    expect(() => resolveId('@buddhilive/dsh-goal/remote/nested')).toThrow(/purity/)
  })

  it('throws on any other @deepseek-ai leak', () => {
    expect(() => resolveId('@buddhilive/dsh-agent')).toThrow(/purity/)
    expect(() => resolveId('@buddhilive/dsh-client-web')).toThrow(/purity/)
  })

  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike', () => {
    expect(() => resolveId('@buddhilive/dsh-client-connection')).toThrow(/purity/)
    expect(() => resolveId('@buddhilive/dsh-client-ui-session')).toThrow(/purity/)
    expect(() => resolveId('@buddhilive/dsh-client-ui-layout/client')).toThrow(/purity/)
  })

  it('admits package-specific requests only for the declaring bundle', () => {
    const requesting = purityResolveId('@buddhilive/dsh-api-session-controller')
    expect(requesting('@buddhilive/dsh-api-gateway/client')).toBeNull()
    expect(() => resolveId('@buddhilive/dsh-api-gateway/client')).toThrow(/purity/)
  })
  ...
  it('externalizes the baseline independently of each package manifest', () => {
    ...
    const plain = clientConfigs('@buddhilive/dsh-client-connection')[0]?.deps as {
    ...
    expect(plain.neverBundle('@buddhilive/dsh-client-store')).toBe(true)
  })
})

describe('client bundle module requests', () => {
  it('requests what the declaration lists', () => {
    const requests = requestedExternals('@buddhilive/dsh-client-fixture', {
      external: ['react', 'react/jsx-runtime', '@buddhilive/dsh-client-ui-slots'],
    })

    expect([...requests].sort()).toEqual([
      '@buddhilive/dsh-client-ui-slots', 'react', 'react/jsx-runtime',
    ])
  })

  it('requests nothing when the declaration is absent', () => {
    expect(requestedExternals('@buddhilive/dsh-client-fixture', {}).size).toBe(0)
  })

  it('rejects a malformed declaration instead of reading past it', () => {
    expect(() => requestedExternals('@buddhilive/dsh-client-fixture', { external: 'react' }))
...
describe('client bundle debug artifacts', () => {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('@buddhilive/dsh-client-ui-goal')
    ...
    const resolved = new URL(source, 'https://dsh.test/plugins/@buddhilive/dsh-client-ui-goal/client.js.map')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('@buddhilive/dsh-host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('@buddhilive/dsh-client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
    ...
    const resolved = new URL(workspaceSource, 'https://dsh.test/plugins/@buddhilive/dsh-client-connection/client.js.map')
