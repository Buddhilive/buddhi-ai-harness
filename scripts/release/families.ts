...
const WORKSPACE_ROOT_PACKAGE = '@buddhilive/dsh-root'
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@deepseek-ai/') && !name.startsWith('@buddhilive/') && name !== 'buddhi-ai') {
        throw new Error(`${normalized} must name an @deepseek-ai, @buddhilive, or buddhi-ai package`)
      }
...
class DshFamily extends ReleaseFamily {
  ...
  readonly installedEntry = { packageName: 'buddhi-ai', binPath: 'lib/bin.js' }
}
