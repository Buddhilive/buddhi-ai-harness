...
import { ToolCallId, ReasoningEffortId } from '@buddhilive/dsh-llm'
import SystemPrompt from '@buddhilive/dsh-system-prompt'
import ToolRuntime, { TOOL_ABORTED_BEFORE_DISPATCH } from '@buddhilive/dsh-tools'
import { assembleContextFor, type Agent } from '@buddhilive/dsh-agent'
...
import type { SubagentStartRequest } from '@buddhilive/dsh-subagent'
...
describe('dsh-tool-subagent background mode', () => {
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @buddhilive/dsh-jobs')
  })
