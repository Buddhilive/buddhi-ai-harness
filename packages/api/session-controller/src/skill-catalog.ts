...
import { SessionQueryError } from '@buddhilive/dsh-session-query'
import { isUserInvocable } from '@buddhilive/dsh-skill'
...
export class SessionSkillCatalog extends TypertRemoteService {
  ...
  async list(request: SkillListRequest, signal: AbortSignal): Promise<SkillListValue> {
    ...
    if (skillRegistry === undefined) {
      throw new RemoteError(
        'gateway/internal',
        'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @buddhilive/dsh-skill',
        {},
      )
    }
