...
import { startMockLlmServer } from '@buddhilive/dsh-llm-mock-server'
...
function createEnvironmentProbeProfile(home: string, project: string): void {
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    ...
    private: true,
    dependencies: {},
    dsh: { profile: { bundles: ['@buddhilive/dsh-base'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), [
    '- insert:',
 ...
 * `@buddhilive/dsh-cmdline` and `commander` through the profile module
...
function createStartupFixture(): StartupFixture {
  ...
  writeFileSync(join(bundleDir, 'startup.mjs'), [
    ...
    "import { parseCmdline } from '@buddhilive/dsh-cmdline'",
...
describe.skipIf(!existsSync(dshBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  ...
  it('reports SDK startup failure when stdin reaches EOF first', async () => {
    ...
    writeFileSync(patch, [
      ...
      '      name: "@buddhilive/dsh-missing-sdk-startup-plugin"',
      '',
    ].join('\n'))
    try {
      ...
      expect(result.stderr).toContain('@buddhilive/dsh-missing-sdk-startup-plugin')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  ...
  it('activates a dependency that gained dsh.bundle in a later update', async () => {
    ...
    try {
      ...
      expect(manifest.dsh.profile.bundles).toEqual(['@buddhilive/dsh-base'])
      ...
      expect(manifest.dsh.profile.bundles).toEqual(['@buddhilive/dsh-base', 'late-bundle'])
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, SPAWN_TIMEOUT_MS * 2 + 30_000)
  ...
  describe('config dump', () => {
    ...
    it('prints the web profile bundle layers without a user layer', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@buddhilive/dsh-agent-loop'")
      expect(stdout).toContain('agents: []')
      expect(stdout).toContain('# == @buddhilive/dsh-base')
      expect(stdout).toContain("name: '@buddhilive/dsh-host-webserver'")
    ...
    it('prints the headless profile without Host or browser layers', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@buddhilive/dsh-headless'")
      expect(stdout).not.toMatch(/name: '@deepseek-ai\/dsh-host-/)
      expect(stdout).not.toContain("name: '@buddhilive/dsh-web-app'")
    ...
    it('prints the exact standalone sdk-minimal tree without dsh-base', async () => {
      ...
      expect(rows.map(row => [row.id, row.name])).toEqual([
        ['sdk-app-startup', '@buddhilive/dsh-sdk-app'],
        ['sdk-jsonrpc-server', '@buddhilive/dsh-sdk-jsonrpc-server'],
        ...
        ['pty', '@buddhilive/dsh-terminal'],
        ['terminal-bash', '@buddhilive/dsh-terminal-bash'],
        ['terminal-pwsh', '@buddhilive/dsh-terminal-bash'],
        ['fs-local', '@buddhilive/dsh-fs-local'],
        ['agent-spine', '@buddhilive/dsh-agent-spine-demo'],
        ['persistent-bash', '@buddhilive/dsh-tool-bash-persistent'],
        ['persistent-pwsh', '@buddhilive/dsh-tool-pwsh-persistent'],
        ['str-replace-editor', '@buddhilive/dsh-tool-str-replace-editor'],
        ['sessions', '@buddhilive/dsh-session-persistence-jsonl'],
      ])
      expect(stdout).toContain('# == @buddhilive/dsh-sdk-minimal')
      expect(stdout).not.toContain('@buddhilive/dsh-base')
      expect(stdout).not.toContain('@buddhilive/dsh-web-app')
    }, SPAWN_TIMEOUT_MS * 2 + 30_000)
