...
const SESSION_PACKAGE = '@buddhilive/dsh-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@buddhilive/dsh-session/types'
 ...
 * top-level declaration (in `@buddhilive/dsh-session`) and any declaration
 * merge inside a `declare module '@buddhilive/dsh-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @buddhilive/dsh-session/known-event-types',
    ' */',
    '',
    '/**',
