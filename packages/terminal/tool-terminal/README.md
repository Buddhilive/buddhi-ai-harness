...
The tools need `ctx.terminals` — a backend must be mounted — and the system-prompt service for the guidance section. Background sends additionally require the jobs service and its model-facing controller (`@buddhilive/dsh-tool-jobs`).
...
The generated [configuration catalog](../../../docs/config-catalog.md#buddhilivedsh-tool-terminal) and [tool catalog](../../../docs/tool-catalog.md#buddhilivedsh-tool-terminal) are the exhaustive sources for config fields and schemas.
...
- [Tool catalog](../../../docs/tool-catalog.md#buddhilivedsh-tool-terminal) — the six generated schemas and result shapes.
...
The six generated schemas are listed in the [`dsh-tool-terminal` catalog section](../../../docs/tool-catalog.md#buddhilivedsh-tool-terminal). Their fixed schema tokens are present whenever this plugin is active; agent-scoped tool filtering may hide them.
...
- **Background mode requires the jobs surface** — `run_in_background` needs both `@buddhilive/dsh-jobs` and its model-facing controller (`@buddhilive/dsh-tool-jobs`); without them the argument is rejected.
