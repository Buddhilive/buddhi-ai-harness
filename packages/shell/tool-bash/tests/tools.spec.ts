...
import { ShellExecutor } from '@buddhilive/dsh-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult } from '@buddhilive/dsh-shell'
import SystemPrompt from '@buddhilive/dsh-system-prompt'
import ToolRuntime, { TOOL_ABORTED, TOOL_ABORTED_BEFORE_DISPATCH } from '@buddhilive/dsh-tools'
...
import type { ApprovalOutcome } from '@buddhilive/dsh-user-approval'
...
describe('background execution through the job runtime', () => {
  ...
  it('fails loud when the job runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @buddhilive/dsh-jobs and @buddhilive/dsh-tool-jobs')
  })
