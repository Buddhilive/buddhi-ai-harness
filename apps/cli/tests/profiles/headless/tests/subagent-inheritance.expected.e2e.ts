...
import { normalizeSessionSnapshot, type NormalizeContext } from '@buddhilive/dsh-session-snapshot'
import { LOADER_SMOKE_TEST_TIMEOUT_MS, runLoaderSmoke } from '@buddhilive/dsh-loader-smoke'
import { createUserMessage, ReasoningEffortId } from '@buddhilive/dsh-llm'
import SessionStore, { SESSION_FORMAT_VERSION, SessionId, type SessionEvent, type SessionHeader } from '@buddhilive/dsh-session'
      ...
      inspect: async (runCwd) => {
        ...
        const runtimeContexts = (content: string): string[] => content.trimEnd().split('\n').flatMap((line) => {
          ...
          if (record.type !== 'user/message'
            ...
            || record.data.source.plugin !== '@buddhilive/dsh-system-prompt') return []
