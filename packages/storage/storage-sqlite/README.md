...
- name: '@buddhilive/dsh-storage-sqlite'
  config:
...
`wal` suits local disks; a rollback-journal mode (`delete`/`truncate`/`persist`) fits filesystems where WAL's shared-memory files do not work, such as network mounts. The generated [configuration catalog](../../../docs/config-catalog.md#buddhilivedsh-storage-sqlite) is the exhaustive source for every accepted field and its JSDoc.
