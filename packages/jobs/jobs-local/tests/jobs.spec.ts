...
import { bindScopeParent, createScope, scopeOf } from '@buddhilive/dsh-scope'
import type { ScopeKey } from '@buddhilive/dsh-scope'
import { JobId } from '@buddhilive/dsh-jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from '@buddhilive/dsh-jobs'
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load @buddhilive/dsh-tool-jobs in its composition)')
  })
