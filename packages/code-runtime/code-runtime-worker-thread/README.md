...
```yaml
- name: '@buddhilive/dsh-code-runtime'
- name: '@buddhilive/dsh-code-runtime-worker-thread'
  config:
...
Every field is validated and defaulted at load; there are no other tunables. The generated [configuration catalog](../../../docs/config-catalog.md#buddhilivedsh-code-runtime-worker-thread) is the exhaustive source for every accepted field.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#buddhilivedsh-code-runtime-worker-thread) — every accepted config field and its source declaration.
