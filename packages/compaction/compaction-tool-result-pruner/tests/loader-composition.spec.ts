  ...
  it('loads and resolves the flat YAML plugin shape', async () => {
    ...
    context.loader.internal = {
      ...
      async import(specifier: string) {
        if (specifier === '@buddhilive/dsh-session-projection') return SessionProjectionRegistry
        if (specifier === '@buddhilive/dsh-token-meter') return TokenMeter
        if (specifier === '@buddhilive/dsh-compaction-tool-result-pruner') return ToolResultPruner
