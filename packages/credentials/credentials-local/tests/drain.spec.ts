...
import { credentialKey, credentialRef } from '@buddhilive/dsh-credentials'
...
vi.mock('@buddhilive/dsh-atomic-write', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@buddhilive/dsh-atomic-write')>()
...
async function setGate(next: Promise<void>): Promise<void> {
  const mocked = await import('@buddhilive/dsh-atomic-write') as unknown as { __setGate: (next: Promise<void>) => void }
