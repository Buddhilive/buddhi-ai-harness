...
import { withFileLock } from '@buddhilive/dsh-atomic-write'
...
describe('initProfile', () => {
  it('creates manifest, user patch layer, and pnpm workspace once, never overwriting', () => {
    ...
    initProfile(dir, ['@buddhilive/dsh-base'])
    ...
    expect(readProfileManifest('t', dir).dsh?.profile?.bundles).toEqual(['@buddhilive/dsh-base'])
...
describe('loadProfile', () => {
  ...
  it('auto-initializes only shipped templates and fails loud otherwise', () => {
    ...
    expect(PROFILE_TEMPLATES.web?.bundles).toContain('@buddhilive/dsh-base')
    ...
    expect(PROFILE_TEMPLATES.acp).toEqual({
      bundles: ['@buddhilive/dsh-base', '@buddhilive/dsh-acp-app'],
      patchReload: 'startup',
    })
    expect(PROFILE_TEMPLATES.sdk).toEqual({
      bundles: ['@buddhilive/dsh-base', '@buddhilive/dsh-sdk-app'],
      patchReload: 'startup',
    })
  ...
  it('normalizes only the exact installation-owned headless bundle tuple', () => {
    const anchor = stageInstallation({
      '@buddhilive/dsh-base': { patch: '[]\n' },
      '@buddhilive/dsh-web-app': { patch: '[]\n' },
      '@buddhilive/dsh-headless': { patch: '[]\n' },
      'custom-bundle': { patch: '[]\n' },
    })
    const home = tmp()
    const stock = resolveProfileDir('headless', home)
    initProfile(stock, [
      '@buddhilive/dsh-base', '@buddhilive/dsh-web-app', '@buddhilive/dsh-headless',
    ])
    ...
    expect(readProfileManifest('t', stock).dsh?.profile).toEqual({
      bundles: ['@buddhilive/dsh-base', '@buddhilive/dsh-headless'],
      patchReload: 'startup',
    })
    ...
    initProfile(custom, [
      '@buddhilive/dsh-base', '@buddhilive/dsh-web-app', '@buddhilive/dsh-headless', 'custom-bundle',
    ])
