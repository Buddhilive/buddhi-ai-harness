...
describe('collectPackageGraph', () => {
  ...
  it('rejects a missing in-repo peer', () => {
    ...
    expect(() => collectPackageGraph(root, ['client'], 'fixture'))
      .toThrow('fixture: @buddhilive/dsh-consumer references missing in-repo peer @buddhilive/dsh-missing')
  })
})
...
describe('renderModuleGraph', () => {
  it('renders the same peer edge in both generated languages', () => {
    const packages = [
      { short: 'provider', name: '@buddhilive/dsh-provider', group: 'core', rel: 'packages/core/provider', deps: [] },
      { short: 'consumer', name: '@buddhilive/dsh-consumer', group: 'core', rel: 'packages/core/consumer', deps: ['provider'] },
    ]
