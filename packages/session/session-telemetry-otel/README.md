...
- id: sessionTelemetry-otel
  name: '@buddhilive/dsh-session-telemetry-otel'
  config:
...
The generated [configuration catalog](../../../docs/config-catalog.md#buddhilivedsh-session-telemetry-otel) is the exhaustive source for every accepted field. Upload authorization is positive and fail-closed: an unknown direct-construction mode fails before transport configuration is read, only `FULL` accepts direct `ctx.sessionTelemetry.emit()` calls, and `FEEDBACK_ONLY` treats only the exact `feedback/record` object already stored in the canonical log as consent.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#buddhilivedsh-session-telemetry-otel) — every accepted config field and its source declaration.
