...
} from '@buddhilive/dsh-typert-registry'
import type {
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@buddhilive/dsh-tools', 'ToolInput')).toBe('@buddhilive/dsh-tools#ToolInput')
    expect(typertPackageKey('@buddhilive/dsh-tools', 'host')).toBe('@buddhilive/dsh-tools#host')
    expect(ctx.typert.get('@buddhilive/dsh-tools#ToolInput')).toMatchObject({
      package: '@buddhilive/dsh-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@buddhilive/dsh-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@buddhilive/dsh-tools', 'host')).toMatchObject({
      key: '@buddhilive/dsh-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@buddhilive/dsh-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@buddhilive/dsh-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@buddhilive/dsh-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@buddhilive/dsh-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@buddhilive/dsh-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@buddhilive/dsh-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@buddhilive/dsh-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@buddhilive/dsh-tools#ToolInput')).not.toBe(projected)
  })
