...
import { bindSnapshotSelector, RemoteError } from '@buddhilive/dsh-client-test-runtime'
...
import { SettingsDescribeMirror } from '@buddhilive/dsh-client-ui-settings/src/client/settings-mirror.ts'
  ...
  describe('BuddhiAI Studio provider', () => {
    function mountBuddhiSection(options: { mutate?: ReturnType<typeof vi.fn> } = {}) {
      ...
      const face = {
        llm: {
          listProviders: vi.fn(() => Promise.resolve(ok([
            { id: 'buddhi-studio', name: 'BuddhiAI Studio' },
          ]))),
          listConfigurableProviders: vi.fn(() => Promise.resolve(ok([
            {
      ...
      const controller = new ModelsSettingsStore(
        ctxWith(face as never), settingsSchema, new SettingsDescribeMirror(ctxWith(face as never)))
    ...
    it('renders BuddhiAI Studio, hides add buttons, and allows editing baseURL', async () => {
      ...
      const injected: ModelsSectionProps = {
        controller,
        useSnapshot: bindSnapshotSelector(controller.store),
        operations: operationsWith(face as never),
        schema: settingsSchema,
        t,
        renderSlot: () => null,
      }
      ...
      await waitFor(() => {
        expect(mutate).toHaveBeenCalledWith(
          'llm-buddhi',
          [{ op: 'set', path: ['baseURL'], value: 'http://127.0.0.1:8765/v1' }],
          1,
        )
      })
    })
