 ...
 * not built (`pnpm --filter @buddhilive/dsh-client-ui-trajectory bundle`).
 */
import { readFileSync } from 'node:fs'
import { resolve } from 'node:path'
import { Context } from '@deepseek-ai/cordis'
import { stubSettingsScope } from '@buddhilive/dsh-client-test-runtime'
import { afterEach, describe, expect, it } from 'vitest'
import { UiConversation } from '@buddhilive/dsh-client-ui-conversation/client'
import { SlotRegistry } from '@buddhilive/dsh-client-ui-renderer/client'

const PLUGIN_ID = '@buddhilive/dsh-client-ui-trajectory'
...
describe('tsdown client artifact', () => {
  ...
  async function loadArtifact() {
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@buddhilive/dsh-client-store', await import('@buddhilive/dsh-client-store')],
      ['@buddhilive/dsh-client-ui-conversation/client', await import('@buddhilive/dsh-client-ui-conversation/client')],
      ['@buddhilive/dsh-client-ui-primitives', await import('@buddhilive/dsh-client-ui-primitives')],
    ])
  ...
  it.skipIf(code === undefined)('mounted as an object plugin, apply registers the view tab on the real ring', async () => {
    ...
    const locale = await import('@buddhilive/dsh-client-locale/client')
