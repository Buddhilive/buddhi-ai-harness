 ...
 * execution pipeline.
 * @module @buddhilive/dsh-tools
 */

import { Context, Service } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@buddhilive/dsh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@buddhilive/dsh-scope'
import type { ToolCallId, ContentBlock, ToolSchema } from '@buddhilive/dsh-llm'
...
import { assertNever, deepFreeze, snapshotJsonValue, type JsonValue } from '@buddhilive/dsh-util-values'
import type { ToolProviderResult } from '@buddhilive/dsh-system-prompt'
import type { CodeRuntime } from '@buddhilive/dsh-code-runtime'
...
// lives in its own UI-facing module; re-export it so `@buddhilive/dsh-tools`
...
declare module '@deepseek-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@buddhilive/dsh-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@buddhilive/dsh-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@buddhilive/dsh-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@buddhilive/dsh-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@buddhilive/dsh-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @buddhilive/dsh-code-runtime-worker-thread) or set tools mode to "native"`)
    }
