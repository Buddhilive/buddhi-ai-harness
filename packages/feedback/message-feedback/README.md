...
```yaml
- id: message-feedback
  name: '@buddhilive/dsh-message-feedback'
  config:
    maxNoteBytes: 8192
```

A note must contain at least one non-whitespace character and fit within the configured byte length; a blank note is rejected with `note-blank` and an oversized one with `note-too-large`. Accepted text is stored exactly as submitted — nothing is trimmed. The generated [configuration catalog](../../../docs/config-catalog.md#buddhilivedsh-message-feedback) is the exhaustive source for every accepted field and its JSDoc.
