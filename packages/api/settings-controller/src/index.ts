 ...
 *
 * @module @buddhilive/dsh-api-settings-controller
 */
...
import {
  ...
  openNativePath,
  openNativeTextFile,
} from '@buddhilive/dsh-native-command'
import type { SettingsDescriptor, SettingsPathOp, SettingsProvider } from '@buddhilive/dsh-settings'
import type {
...
export class SettingsController extends TypertRemoteService {
  ...
  private provider(): SettingsProvider {
    ...
    if (settings === undefined) {
      throw new RemoteError(
        'gateway/internal',
        'settings service is absent: this deployment does not mount a settings provider (e.g. @buddhilive/dsh-settings-file) in its composition',
        {},
      )
    }
    return settings
