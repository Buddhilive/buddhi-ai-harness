...
- name: '@buddhilive/dsh-fs-sandbox'
  config:
    cwd: /absolute/path/to/workspace
- name: '@buddhilive/dsh-tool-fs'
```

The backend's config is the local backend's unchanged (`cwd` resolution default and `diffBasisMaxBytes` overwrite bound); the [configuration catalog](../../../docs/config-catalog.md#buddhilivedsh-fs-sandbox) is the exhaustive source.
