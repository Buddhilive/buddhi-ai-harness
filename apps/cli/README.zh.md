...
`dsh.profile.bundles` 中列出的组合包先从 dsh 安装目录解析（`@buddhilive/dsh-base`、`@buddhilive/dsh-web-app`、`@buddhilive/dsh-headless`、`@buddhilive/dsh-sdk-app`、`@buddhilive/dsh-sdk-minimal`、`@buddhilive/dsh-acp-app`），再从 profile 自身的 `node_modules` 解析；pnpm 会将树外插件安装到该目录。
