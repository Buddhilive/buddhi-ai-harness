...
生成的[配置目录](../../../docs/config-catalog.zh.md#buddhilivedsh-session-telemetry-otel)是每个受支持字段的穷尽式真源。上传授权采用显式许可，且为 fail-closed：通过直接构造传入未知模式时会在读取传输配置前失败，只有 `FULL` 接受对 `ctx.sessionTelemetry.emit()` 的直接调用，`FEEDBACK_ONLY` 只把权威日志中已存储的精确 `feedback/record` 对象视为同意。
...
- [生成配置目录](../../../docs/config-catalog.zh.md#buddhilivedsh-session-telemetry-otel)——每个受支持配置字段及其源声明。
