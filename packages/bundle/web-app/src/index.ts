/**
 * @buddhilive/dsh-web-app — the browser-surface bundle's runtime glue plugin
...
import { addHarnessSourceSection } from '@buddhilive/dsh-app-boot'
import type {} from '@buddhilive/dsh-client-connection'
import * as FrontendStatic from '@buddhilive/dsh-host-frontend-static'
import { launchEnvironmentOf } from '@buddhilive/dsh-launch-environment'
import { scrubbedParentEnv } from '@buddhilive/dsh-subprocess'
...
function resolveDistIndex(): string {
  ...
  try {
    return join(dirname(require.resolve('@buddhilive/dsh-web-frontend/package.json')), 'dist', 'index.html')
  } catch {
    /* v8 ignore next 2 -- reachable only when the frontend package is absent from the checkout */
    throw new Error('web-app: @buddhilive/dsh-web-frontend is not resolvable from this composition')
  }
}
