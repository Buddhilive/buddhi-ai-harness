...
describe('npm resolution benchmark', () => {
  ...
  it('combines installed metadata with current publishable workspace fields', () => {
    ...
    writeJson(root, 'apps/cli/package.json', {
      ...
      version: '0.1.0',
      dependencies: { '@buddhilive/dsh-child': 'workspace:^', external: '^2.0.0' },
    ...
    expect(index.get('@deepseek-ai/dsh')?.get('0.1.0')).toEqual({
      name: '@deepseek-ai/dsh',
      version: '0.1.0',
      dependencies: { '@buddhilive/dsh-child': '^0.1.0', external: '^2.0.0' },
    })
  })
    ...
    try {
      const index: RegistryIndex = new Map([
        ['@deepseek-ai/dsh', new Map([['0.1.0', {
          name: '@deepseek-ai/dsh',
          version: '0.1.0',
          peerDependencies: { '@buddhilive/dsh-peer': '1.0.0' },
        }]])],
        ['@buddhilive/dsh-peer', new Map([['1.0.0', {
          name: '@buddhilive/dsh-peer',
          version: '1.0.0',
        }]])],
      ])
      ...
      expect(result.archiveRequests).toBe(0)
      expect(result.packageLock.packages['node_modules/@buddhilive/dsh-peer']?.version).toBe('1.0.0')
    } finally {
