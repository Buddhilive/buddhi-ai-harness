 ...
 * `@buddhilive/dsh-*` listed 49 candidate globs and `@buddhilive/dsh-*\/invariant`
 ...
 * @returns Every directory whose manifest names a `@buddhilive/dsh-` package and that carries `src`.
 */
 ...
 * `@buddhilive/dsh-<directory>`. Packages named after something other than
 ...
 *
 * @returns Declared names of every `@buddhilive/dsh-` package carrying a `src` directory.
 */
...
export function mappedSpecifiers(text: string): Set<string> {
  ...
  for (const match of text.matchAll(/^\s*"(@buddhilive\/dsh-[^"/]+)":/gm)) {
    const key = match[1]
...
function handWrittenSpecifiers(text: string): Set<string> {
  ...
  const keys = new Set<string>()
  for (const match of outside.matchAll(/^\s*"(@(?:deepseek-ai|buddhilive)\/[^"]+)":/gm)) {
    const key = match[1]
    if (key !== undefined) keys.add(key)
  }
  return keys
