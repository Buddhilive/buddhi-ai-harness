...
```yaml
- id: sandbox
  name: '@buddhilive/dsh-sandbox-local'     # the per-platform backend provider (ctx.sandbox)
- id: sandbox-policy
  name: '@buddhilive/dsh-sandbox-policy'    # the deployment default mode and workspace-write root
  config:
...
- id: bash
  name: '@buddhilive/dsh-bash-sandbox'      # the confined executor behind ctx.shell
```
