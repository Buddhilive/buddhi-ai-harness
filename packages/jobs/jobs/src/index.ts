 ...
 * process-local registry lives in `@buddhilive/dsh-jobs-local`.
 * @module @buddhilive/dsh-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@buddhilive/dsh-jobs is the abstract job registry seam; load an implementation such as @buddhilive/dsh-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
