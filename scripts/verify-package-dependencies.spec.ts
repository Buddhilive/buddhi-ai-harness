...
function facts(manifest: PackageDependencyManifest): PackageDependencyFacts {
  return {
    ...
    workspaceNames: new Set([
      CORDIS,
      '@buddhilive/dsh-runtime',
      '@buddhilive/dsh-types',
      '@buddhilive/dsh-stale',
      '@deepseek-ai/schemastery',
    ]),
    allSourceUses: new Map([
      ['@buddhilive/dsh-runtime', ['packages/core/probe/src/index.ts']],
      ['@buddhilive/dsh-types', ['packages/core/probe/src/types.ts']],
    ]),
    hostRuntimeSourceUses: new Map([
      ['@buddhilive/dsh-runtime', ['packages/core/probe/src/index.ts']],
    ]),
    hostRuntimeExportUses: [{
      packageName: '@buddhilive/dsh-runtime',
      specifier: '@buddhilive/dsh-runtime',
      exportName: 'runtimeValue',
      sourcePath: 'packages/core/probe/src/index.ts',
      line: 1,
      column: 10,
      sourceLine: "import { runtimeValue } from '@buddhilive/dsh-runtime'",
    }],
...
describe('package dependency scope', () => {
  it('keeps the measured Host relay roster explicit', () => {
    ...
    expect(PACKAGE_DEPENDENCY_POLICY.safeHostDependencyExports['@buddhilive/dsh-deque']).toEqual(['Deque'])
    expect(PACKAGE_DEPENDENCY_POLICY.safeHostDependencyExports['@deepseek-ai/schemastery']).toEqual(['default'])
    expect(PACKAGE_DEPENDENCY_POLICY.safeHostDependencyExports['@buddhilive/dsh-session/types']).toBeUndefined()
    expect(PACKAGE_DEPENDENCY_POLICY.safeHostDependencyExports['@buddhilive/dsh-typert-protocol']).toBeUndefined()
    expect(PACKAGE_DEPENDENCY_POLICY.peerRequiredHostExports['@buddhilive/dsh-scope']).toEqual([
      'carrierKeyOf', 'scopeOf', 'scopeTarget',
    ])
    expect(PACKAGE_DEPENDENCY_POLICY.peerRequiredHostExports['@buddhilive/dsh-typert-protocol']).toBeUndefined()
  })
...
describe('dependency sections', () => {
  it('does not leak repository configuration into captured dependency facts', () => {
    const manifest: PackageDependencyManifest = {
      name: '@buddhilive/dsh-client-locale',
      dependencies: { '@buddhilive/dsh-runtime': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@buddhilive/dsh-types': 'workspace:^' },
      peerDependencies: { [CORDIS]: 'workspace:^' },
    }
    const base = facts(manifest)
    const subject: PackageDependencyFacts = {
      ...base,
      workspaceNames: new Set([...base.workspaceNames, '@buddhilive/dsh-api-remotes']),
    }
  ...
  it('requires non-workspace Host runtime imports in dependencies', () => {
    const manifest: PackageDependencyManifest = {
      name: '@buddhilive/dsh-probe',
      dependencies: { '@buddhilive/dsh-runtime': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@buddhilive/dsh-types': 'workspace:^', external: '^1.0.0' },
      peerDependencies: { [CORDIS]: 'workspace:^' },
    }
    const subject: PackageDependencyFacts = {
      ...facts(manifest),
  ...
  it('accepts Host dependencies, development-only inputs, and shared Cordis', () => {
    const manifest: PackageDependencyManifest = {
      name: '@buddhilive/dsh-probe',
      dependencies: {
        '@buddhilive/dsh-runtime': 'workspace:^',
        '@deepseek-ai/schemastery': 'workspace:^',
        external: '^1.0.0',
      },
      devDependencies: {
        '@buddhilive/dsh-types': 'workspace:^',
        [CORDIS]: 'workspace:^',
      },
      peerDependencies: { [CORDIS]: 'workspace:^' },
    }
  ...
  it('lists managed Host runtime dependencies for fix review', () => {
    const subject = facts({ name: '@buddhilive/dsh-probe' })
    expect(formatManagedRuntimeDependencies({
      ...
      '  @buddhilive/dsh-probe -> @buddhilive/dsh-runtime: @buddhilive/dsh-runtime#runtimeValue',
    ])
  })
  ...
  it('reports an unapproved Host runtime export without rewriting its dependency section', () => {
    ...
    expect(safetyViolations).toEqual([
      'packages/core/probe/src/index.ts:1:10: @buddhilive/dsh-runtime#runtimeValue is not classified as '
      + 'safe or peer-required — import { runtimeValue } from \'@buddhilive/dsh-runtime\'',
    ])
    expect(fixPackageDependencies('/unused', state)).toEqual([])
    expect(manifest.dependencies).toEqual({ '@buddhilive/dsh-runtime': 'workspace:^' })
  })
  ...
  it('keeps an edge as a peer when one imported export requires shared identity', () => {
    ...
    const subject: PackageDependencyFacts = {
      ...facts(manifest),
      peerRequiredHostDependencies: new Set(['@buddhilive/dsh-runtime']),
    }
    expect(collectHostDependencyExportPolicyViolations(
      [subject],
      subject.workspaceNames,
      {
        safeHostDependencyExports: {},
        peerRequiredHostExports: {
          '@buddhilive/dsh-runtime': ['runtimeValue'],
        },
      },
    )).toEqual([])
  ...
  it('reports wrong sections, workspace ranges, and stale peer metadata', () => {
    const manifest: PackageDependencyManifest = {
      name: '@buddhilive/dsh-probe',
      dependencies: { '@buddhilive/dsh-types': 'workspace:*' },
      devDependencies: { [CORDIS]: 'workspace:^', '@buddhilive/dsh-runtime': 'workspace:^' },
      peerDependencies: { [CORDIS]: 'workspace:*', '@buddhilive/dsh-runtime': 'workspace:^' },
      peerDependenciesMeta: { '@buddhilive/dsh-missing': { optional: true } },
    }
    const state = {
      facts: [facts(manifest)], packages: [], policyViolations: [], workspaceNames: facts(manifest).workspaceNames,
    }
    ...
    expect(violations).toEqual(expect.arrayContaining([
      expect.stringContaining('@buddhilive/dsh-runtime'),
      expect.stringContaining('@buddhilive/dsh-types'),
      expect.stringContaining(`${CORDIS} must be matching peerDependencies + devDependencies`),
      expect.stringContaining('dependencies.@buddhilive/dsh-types must use workspace:^'),
      expect.stringContaining('peerDependenciesMeta.@buddhilive/dsh-missing has no matching'),
    ]))
  })
  ...
  it('repairs owned relationships without changing unrelated dependencies', () => {
    ...
    const manifest: PackageDependencyManifest = {
      ...
      peerDependencies: {
        [CORDIS]: 'workspace:^',
        '@buddhilive/dsh-runtime': 'workspace:^',
        '@buddhilive/dsh-stale': 'workspace:^',
      },
      peerDependenciesMeta: { '@buddhilive/dsh-stale': { optional: true } },
    }
